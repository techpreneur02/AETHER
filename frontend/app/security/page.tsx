...
export default function SecurityPage() {
  ...
  useEffect(() => {
    ...
    if (token && projectId) {
      Promise.all([
        ...
        .then(([loadedRules, loadedTopology]) => {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus("");
    try {
      const rule = await createSecurityRule(token, projectId, {
        name: name.trim(),
        action,
        protocol,
        source: source.trim(),
      ...
      setStatus(error instanceof Error ? error.message : "Unable to add rule");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function remove(ruleId: string) {
    ...
    try {
      await deleteSecurityRule(token, projectId, ruleId);
      setRules((current) => current.filter((rule) => rule.id !== ruleId));
      setStatus("Security rule removed");
    } catch (error) {
      setStatus(
        error instanceof Error ? error.message : "Unable to remove rule",
      );
    }
  }
  ...
  return (
    <main className="imports-shell">
      ...
      <section className="imports-workspace">
        <span className="eyebrow">CONTROL / SECURITY</span>
        <h1>Make policy visible and reviewable.</h1>
        <p>
          Record project firewall intent before it becomes a production change.
        </p>
        <section className="import-card panel">
          ...
          <form className="security-form" onSubmit={submit}>
            ...
            <select
              value={action}
              onChange={(event) =>
                setAction(event.target.value as SecurityRule["action"])
              }
            >
            ...
            <select
              value={protocol}
              onChange={(event) =>
                setProtocol(event.target.value as SecurityRule["protocol"])
              }
            >
          ...
          <table className="ip-table security-table">
            <thead>
              <tr>
                <th>Rule</th>
                <th>Action</th>
            ...
            <tbody>
              {rules.map((rule) => (
                <tr key={rule.id}>
                  ...
                  <td>{rule.port}</td>
                  <td>
                    {topology.nodes.find((node) => node.id === rule.device_id)
                      ?.name ?? "Global"}
                  </td>
                  <td>
                    <button
              ...
              {!rules.length && (
                <tr>
                  <td colSpan={8} className="empty-allocations">
                    <LockKeyhole size={20} /> No security rules defined.
                  </td>
                </tr>
              )}
            </tbody>
